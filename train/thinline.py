...
TARGET_CLASS = 4
MODEL_NAME = "..\\models\\thinline.pth"


# 加载数据
def load_data(batch_size=20, display=False):
    ...
    train_data = torchvision.datasets.ImageFolder(root='..\\train_data\\thinline',
                                                  transform=trans)
...
def patchCheck(model, img):
    if not img.shape[0] == 48 or not img.shape[1] == 48:
        return -1, 0
    feature = transforms.ToTensor()(img).unsqueeze(dim=0)
    _pred = model(feature).squeeze(dim=0)
    pred_prob = nn.Softmax(dim=0)(_pred).unsqueeze(dim=0)
